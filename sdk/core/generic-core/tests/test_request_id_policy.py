...
from generic.core.pipeline.policies import RequestIdPolicy
...
def test_request_id_policy(
    ...
    expected_header_name = "client-request-id"
    if request_id_header_name == "_unset":
        expected_header_name = request_id_policy._request_id_header_name
...
def test_request_id_already_exists(http_request):
    ...
    request = http_request("GET", "http://localhost/")
    request.headers[request_id_policy._request_id_header_name] = "VALUE"
    pipeline_request = PipelineRequest(request, PipelineContext(None))
    request_id_policy.on_request(pipeline_request)
    assert request.headers[request_id_policy._request_id_header_name] == "VALUE"
