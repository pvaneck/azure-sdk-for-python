...
from generic.core.pipeline import PipelineResponse, PipelineRequest, PipelineContext
from generic.core.pipeline.policies import DistributedTracingPolicy, UserAgentPolicy
...
def test_distributed_tracing_policy_solo(http_request, http_response):
    ...
    with FakeSpan(name="parent") as root_span:
        ...
        request.headers[DistributedTracingPolicy._REQUEST_ID] = "some client request id"

        pipeline_request = PipelineRequest(request, PipelineContext(None))
        policy.on_request(pipeline_request)

        response_headers = request.headers
        response_headers[DistributedTracingPolicy._RESPONSE_ID] = "some request id"
        response = create_http_response(http_response, request, None, status_code=202, headers=response_headers)
    ...
    assert network_span.attributes.get(DistributedTracingPolicy._RESPONSE_ID) == "some request id"
    assert network_span.attributes.get(DistributedTracingPolicy._REQUEST_ID) == "some client request id"
    ...
    assert network_span.attributes.get("http.url") == "http://localhost/temp?query=query"
    assert network_span.attributes.get(DistributedTracingPolicy._REQUEST_ID) == "some client request id"
    assert network_span.attributes.get(DistributedTracingPolicy._RESPONSE_ID) is None
    assert network_span.attributes.get("http.user_agent") is None
...
def test_distributed_tracing_policy_attributes(http_request, http_response):
    ...
    with FakeSpan(name="parent") as root_span:
        ...
        response = create_http_response(http_response, request, None, status_code=202, headers=request.headers)
