...
def test_bearer_policy_access_token_info_caching(http_request):
    ...
    refreshable_token = AccessTokenInfo("token", int(time.time() + 3600), refresh_on=int(time.time() - 65))
    credential.get_token_info.reset_mock()
...
def test_need_new_token_with_jitter():
    ...
    with patch('azure.core.pipeline.policies._authentication.time') as mock_time:
        ...
        policy._refresh_jitter = 60
        assert not policy._need_new_token

        # Test with a token that expires in 200 seconds
        policy._token = AccessToken("", 1200)  # 1000 + 200
...
def test_need_new_token_with_refresh_on_and_jitter():
    ...
    policy._token = AccessTokenInfo("", expiry_time, refresh_on=refresh_time)
    ...
    close_expiry = now + 30
    policy._token = AccessTokenInfo("", close_expiry, refresh_on=close_expiry - 5)
    ...
    policy._token = AccessTokenInfo("", now, refresh_on=now - 5)
    policy._refresh_jitter = 60
...
def test_need_new_token_jitter_boundary_conditions():
    ...
    with patch('azure.core.pipeline.policies._authentication.time') as mock_time:
        ...
        policy._token = AccessToken("", 1300)  # 1000 + 300
        ...
        policy._token = AccessToken("", 1299)  # 1000 + 299
        ...
        policy._token = AccessTokenInfo("", 4600, refresh_on=1000)  # expires at 1000 + 3600, refresh at 1000
        policy._refresh_jitter = 0
        # Effective refresh time is 1000 + 0 = 1000, so 1000 >= 1000 is True
        assert policy._need_new_token

        # Test refresh_on with positive jitter (delays refresh)
        policy._token = AccessTokenInfo("", 4600, refresh_on=995)  # expires at 1000 + 3600, refresh at 995
        policy._refresh_jitter = 10
